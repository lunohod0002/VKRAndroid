...
class StationViewModel @Inject constructor(
    ...
    fun getCurrentStation(){
        ...
        lifecycleScope.launch {
            val cell = withContext(Dispatchers.IO) { viewModel.fetchCurrentLocation() }
            if (cell != null) {
                val args = StationFragmentArgs(
                    STATION = StationData(cell.station!!, cell.branch!!)
                )
                navController.navigate(
                    R.id.screen_station, args.toBundle(), navOptions {
                        launchSingleTop = true
                    }
                )
            } else {
                Toast.makeText(
                    this@MainActivity, "Не удалось определить текущую станцию",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
